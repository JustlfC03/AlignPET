...
def get_all_paths_var(data_dir, var_dir, mode,modality):
    mode_path = os.path.join(data_dir, mode)
    var_path = os.path.join(var_dir, mode,modality)
    mri_paths = []
    pet_paths = []
    var_conditionpath = []
...
def load_data(mri_paths, pet_paths, varconditionpath,mri_wenli_path):

    mri_path, pet_path = mri_paths, pet_paths
    mri = nib.load(mri_path).get_fdata()
    pet = nib.load(pet_path).get_fdata()
    wenli = nib.load(mri_wenli_path).get_fdata()
    var_condition = np.load(varconditionpath)


    mri = torch.tensor(mri, dtype=torch.float32)
    pet = torch.tensor(pet, dtype=torch.float32)
    wenli = torch.tensor(wenli, dtype=torch.float32)
    var_condition = torch.tensor(var_condition, dtype=torch.float32)
...
def get_all_MRIwenliattention_paths(data_dir,var_dir, mode,modality):
    mri_paths, pet_paths,_ = get_all_paths_var(data_dir, var_dir, mode,modality)
    wenli_dir = os.path.join(data_dir,'wenli',mode,modality)
    os.makedirs(wenli_dir, exist_ok=True)
    for i in mri_paths:
        name = i.split('/')[-1]
        wenli_path = os.path.join(wenli_dir,name)
...
class myDataset3D(Dataset):
    def __init__(self, data_dir, var_dir, mode, modality):
        mri_path, pet_path, var_conditionpath = get_all_paths_var(data_dir, var_dir, mode,modality)
        self.modality = modality
        self.mode = mode
        self.wenli_dir = os.path.join(data_dir, 'wenli', mode, modality)
        if not os.path.exists(self.wenli_dir):
            get_all_MRIwenliattention_paths(data_dir, var_dir, mode,modality)
    ...
    def __getitem__(self, idx):
        ...
        if self.modality == 'T2':
            cerebral_cortex_dir = os.path.join('',self.mode,'T2',name)
            cerebral_cortex = nib.load(cerebral_cortex_dir).get_fdata()
            cerebral_cortex = torch.from_numpy(cerebral_cortex.astype(np.float32))
            cerebral_cortex = cerebral_cortex.permute((2, 0, 1))
            cerebral_cortex = F.pad(cerebral_cortex, pad=(64, 64, 64, 64), mode='constant', value=0.0)
            cerebral_cortex = cerebral_cortex.unsqueeze(0)
        else:
            cerebral_cortex = 0
        ...
        wenli_path = os.path.join(self.wenli_dir,name)

        mri, pet, varcondition,mri_wenli = load_data(mri_path, pet_path, var_conditionpath,wenli_path)
        ...
        mri = F.pad(mri, pad=(64, 64, 64, 64), mode='constant', value=0.0)
        pet = F.pad(pet, pad=(64, 64, 64, 64), mode='constant', value=0.0)
        mri_wenli = F.pad(mri_wenli, pad=(64, 64, 64, 64), mode='constant', value=0.0)

        mri_tensor =mri.unsqueeze(0)
        pet_tensor = pet.unsqueeze(0)
        mri_wenli = mri_wenli.unsqueeze(0)

        #print(mri_wenli.shape)

        return mri_tensor, pet_tensor, varcondition,mri_wenli,name,cerebral_cortex
