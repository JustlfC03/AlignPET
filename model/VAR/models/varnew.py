    ...
    def autoregressive_infer_cfg(
        ...
        for si, pn in enumerate(self.patch_nums):  # si: i-th segment
            ...
            f_hat, next_token_map, h_up = self.vae_quant_proxy[0].get_next_autoregressive_input(si,
                                                                                                len(self.patch_nums),
                                                                                                f_hat, h_BChw)
            #muti_scal.append(self.vae_proxy[0].fhat_to_img(f_hat))
        ...
        return self.vae_proxy[0].fhat_to_img(f_hat), f_out#,muti_scal

    def forward(self, x_BLCv_wo_first_l: torch.Tensor, inpMRI
                ):
        """
        :param label_B: label_B
        :param x_BLCv_wo_first_l: teacher forcing input (B, self.L-self.first_l, self.Cvae)
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            sos = inpMRI.view(inpMRI.shape[0],-1)
            cond_BD = sos
            sos = sos.unsqueeze(1).expand(B, self.first_l, -1) + self.pos_start.expand(B, self.first_l, -1)
