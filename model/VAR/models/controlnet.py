...
class ControlNetConditioningEmbedding(nn.Module):
    def __init__(self):
        super().__init__()
        # 初始卷积 (保持尺寸)
        self.conv_in = nn.Conv2d(320, 128, kernel_size=3, padding=1)
    ...
    def forward(self, x):
        x = self.conv_in(x)  # [16, 320, 16,16] → [16, 128, 16,16]
        x = self.upsample(x)  # [16, 128, 16,16] → [16, 128, 64,64]
        return x
...
class UpsampleModel(nn.Module):
    def __init__(self):
        ...
        self.upsample_blocks = nn.Sequential(
            ...
            # 128x128 -> 256x256
            self._make_upsample_block(16, 8)
        )
    ...
    def forward(self, x):
        x = self.initial_conv(x)     # (b,128,16,16)
        x = self.upsample_blocks(x)   # (b,8,256,256)
