    ...
    def forward(self, f, x, y):
        if self.current_stage == self.num_stage:
            ...
            if self.current_stage == 1:
                ...
                return self.steps4_order4_implicit(f[-self.current_stage + 4], f[-self.current_stage + 3],
...
class MyDecoder(nn.Module):
    def __init__(self,
        ...
        for s in range(1, n_stages_encoder + 1):
            ...
            for j in range(len(encoder.output_channels) - s + 1):
                scale_for_upsample = [scale_for_upsample[k] * encoder.strides[j][k] for k in
                                      range(len(encoder.strides[j]))]
            upsamples.append(
                upsample_op(input_features_below, input_features_skip, 1, scale_for_upsample, bias=conv_bias))
        ...
        for s in range(1, n_stages_encoder + 1):
            if s != n_stages_encoder:
                ...
                up_upperlayer = None
            stages.append(
                single_decoder(n_stages_encoder, s, upsamples[s - 1], up_upperlayer, do_nothing, add_op(nn.ReLU())))
    ...
    def forward(self, ori_input, skips):
        ...
        y = torch.zeros(b, c, h, w, z).to(ori_input.device)
        seg_outputs = []
        f = []
